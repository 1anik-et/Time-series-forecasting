...
data = btc.history(period="1y", interval="1m")  # You can set the desired period
...
# data['Unix_timestamp'] = (data.index.view('int64') // 10**6)
# data = data.rename(columns=columns_dict)
# data = data.rename(columns={'Unix_timestamp': 'Timestamp'})
# data = pd.read_csv("C:\\Users\\raman\\Downloads\\okex_btcusdt_kline_1m.csv\\okex_btcusdt_kline_1m.csv")
# data = pd.read_csv("C:\\Users\\raman\\PycharmProjects\\pythonProject1\\pytorch\\finace\\btcusd_1-min_data.csv")
data = pd.read_csv("C:\\Users\\HAKAI\\OneDrive\\Desktop\\Time_series_forecasting\\btc_15m_data_2018_to_2025.csv")
data = data.rename(columns=columns_dict)
...
def fixed_iteritems(cls, *args, **kwargs):
    try:
        return cls.items(*args, **kwargs)
    except AttributeError:
        return cls.iteritems(*args, **kwargs)

finta.finta.pd.Series.iteritems = fixed_iteritems
...
data_finta['open'] = data['Open']
data_finta['high'] = data['High']
data_finta['low'] = data['Low']
data_finta['close'] = data['Close']
data_finta['volume'] = data['Volume']
print(data_finta)
...
for column in data_min.columns:
    nan_indices = data_min[column][data_min[column].isna()].index

    if not nan_indices.empty:  # Make sure there are NaN values
        max_index_column = nan_indices[-1]
        if plot_data_process:
...
start_index = data_min.dropna().index[0]
...
new_columns_order = ['Close', 'Volume', 'Open', 'High', 'Low','TRIX', 'VWAP', 'MACD',
...
def train_validation_test_split(data, val_percentage=0.2, test_percentage=0.2):
...
# split the data
val_percentage = 0.0068
test_percentage = 0.0068
...
def greedy_decode(model, src, bptt_src, pred_len, overlap):
    ...
    Returns:
        greedy_output: tensor, shape [pred_len, 1, E], holds the predictions based on src
...
torch.save(best_model.state_dict(), 'model_state_dict.pth')